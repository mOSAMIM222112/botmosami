  ...
  try {
    ...
    try {
      ...
      if (chat) {
        ...
        if (!("sWelcome" in chat))
          chat.sWelcome = "HELLO WELCOM TO BPT MOSAMIM";
        if (!("sBye" in chat)) chat.sBye = "GOODBYE SEE YOU LATER";
...
—◉ *Contacto para apelaciones:* wa.me/620986991
╚═════════════════════╝
               `.trim();
              m.reply(messageText);
        ...
        if (
          ...
          mconn.conn.reply(
            m.chat,
            `*[ 💎 ] Sus diamantes (limites) se han agotado, puede adquirir más con el comando ${usedPrefix}buyall*`,
            m,
          );
          continue;
        }
        if (plugin.level > _user.level) {
          mconn.conn.reply(
            m.chat,
            `*[ 💠 ] Se require tener el nivel ${plugin.level} para poder usar esté comando. Tú nivel actual es ${_user.level}, usa el comando ${usedPrefix}lvl para subir tu nivel con exp.*`,
            m,
          );
          continue;
        }
        ...
        try {
          ...
          if (m.limit) {
            m.reply(
              "*[ 💎 ] Se utilizarón " + +m.limit + " diamante(s) (limites).*",
            );
          }
        }
        break;
    ...
    if (nk.isGroup == false) {
      if (nk.status == "offer") {
        const callmsg = await mconn.conn.reply(
          nk.from,
          `hello *@${nk.from.split("@")[0]}*, las ${
            nk.isVideo ? "videollamadas" : "llamadas"
          } Not allowed, you will be blocked.\n-\nIf you connect by mistake, contact my creator to unblock you!`,
          false,
          { mentions: [nk.from] },
        );
...
global.dfail = (type, m, conn) => {
  const msg = {
    rowner: "*[ ⚠️ ] This command can only be used by the owner of the Bot.*",
    owner: "*[ ⚠️ ] This command can only be used by the owner of the Bot.*",
    mods: "*[ ⚠️ ] Este comando solo puede ser utilizado por moderadores y el/la propietario(a) (owner) del Bot.*",
    premium:
      "*[ ⚠️ ] Este comando solo puede ser utilizado por usarios premium y el/la propietario(a) (owner) del Bot.*",
    group: "*[ ⚠️ ] Este comando solo puede ser utilizado en grupos.*",
    private:
      "*[ ⚠️ ] Este comando solo puede ser utilizado en el chat privado del Bot.*",
    admin: "*[ ⚠️ ] Este comando solo puede ser usado por admins del grupo.*",
    botAdmin:
      "*[ ⚠️ ] Para poder usar este comando es necesario que yo sea admin.*",
    unreg:
      "*[ 🛑 Hey!! Alto, no estas registrado 🛑 ]*\n\n*—◉ Para poder usar este comando debes registrarte, usa el comando:*\n*➣ #verificar nombre.edad*",
    restrict:
      "*[ ⚠️ ] Este comando esta restringido/desactivado por desición del propietario(a) (owner) del Bot.*",
  }[type];
