...
const handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  if (!text)
    throw `*[❗] اڪتب مادا تࢪيد ان تبحث ؏ـليه في شات جي بي تي او التحدث مـ؏ـه*\n\n\n`;
  try {
    ...
    try {
      ...
      if (
        botIA222.data.choices[0].text == "error" ||
        botIA222.data.choices[0].text == "" ||
        !botIA222.data.choices[0].text
      )
        return XD; // causar error undefined para usar otra api
      m.reply(botIA222.data.choices[0].text.trim());
    } catch {
      try {
        conn.sendPresenceUpdate("composing", m.chat);
        const syms1 = `Actuaras como un Bot de WhatsApp el cual fue creado por BrunoSobrino, tu seras The Mystic - Bot.`;
        const fgapi1 = await fetch(
          `https://api-fgmods.ddns.net/api/info/openai?text=${text}&symsg=${syms1}&apikey=XlwAnX8d`,
        );
        const fgjson1 = await fgapi1.json();
        if (
          fgjson1.result == "error" ||
          fgjson1.result == "" ||
          !fgjson1.result
        )
          return XD; // causar error undefined para lanzar msg de error
        m.reply(`${fgjson1.result}`.trim());
      } catch {
        try {
          conn.sendPresenceUpdate("composing", m.chat);
          const vihangayt1 = await fetch(
            `https://vihangayt.me/tools/chatgpt?q=${text}`,
          );
          const vihangaytjson1 = await vihangayt1.json();
          if (
            vihangaytjson1.data == "error" ||
            vihangaytjson1.data == "" ||
            !vihangaytjson1.data
          )
            return XD; // causar error undefined para usar otra api
          ...
          try {
            conn.sendPresenceUpdate("composing", m.chat);
            const vihangayt2 = await fetch(
              `https://vihangayt.me/tools/chatgpt2?q=${text}`,
            );
            const vihangaytjson2 = await vihangayt2.json();
            if (
              vihangaytjson2.data == "error" ||
              vihangaytjson2.data == "" ||
              !vihangaytjson2.data
            )
              return XD; // causar error undefined para usar otra api
            ...
            try {
              conn.sendPresenceUpdate("composing", m.chat);
              const vihangayt3 = await fetch(
                `https://vihangayt.me/tools/chatgpt3?q=${text}`,
              );
              const vihangaytjson3 = await vihangayt3.json();
              if (
                vihangaytjson3.data == "error" ||
                vihangaytjson3.data == "" ||
                !vihangaytjson3.data
              )
                return XD; // causar error undefined para usar otra api
              ...
              try {
                ...
                try {
                  ...
                  const replacedText = sextS
                    .replace(searchString2, replacementString2)
                    .trim();
                  m.reply(replacedText);
                } catch {
                  try {
                    conn.sendPresenceUpdate("composing", m.chat);
                    const akuariapi2 = await fetch(
                      `https://api.akuari.my.id/ai/gpt?chat=${text}`,
                    );
                    const akuariapijson2 = await akuariapi2.json();
                    if (
                      akuariapijson2.respon == "error" ||
                      akuariapijson2.respon == "" ||
                      !akuariapijson2.respon
                    )
                      return XD; // causar error undefined para lanzar msg de error
                    const akuariapiresult2 = await translate(
                      `${akuariapijson2.respon}`,
                      { to: "es", autoCorrect: true },
                    );
                    m.reply(akuariapiresult2.text.trim());
                  } catch {
                    try {
                      conn.sendPresenceUpdate("composing", m.chat);
                      const akuariapi1 = await fetch(
                        `https://api.akuari.my.id/ai/gbard?chat=${text}`,
                      );
                      const akuariapijson1 = await akuariapi1.json();
                      if (
                        akuariapijson1.respon == "error" ||
                        akuariapijson1.respon == "" ||
                        !akuariapijson1.respon
                      )
                        return XD; // causar error undefined para usar otra api
                      const akuariapiresult1 = await translate(
                        `${akuariapijson1.respon}`,
                        { to: "es", autoCorrect: true },
                      );
...
handler.command =
  /^(openai|gpt|chatgpt|ia|robot|openai2|chatgpt2|ia2|robot2|Mystic|MysticBot)$/i;
